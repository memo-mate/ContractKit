...
const ContractPanel: React.FC<ContractPanelProps> = ({
  ...
  const parseTable = (tableHtml: string, position: number, validIssues: ContractIssue[]): JSX.Element => {
    // 创建临时DOM元素解析表格
    ...
    // 获取表格所有单元格内容
    const cells: HTMLTableCellElement[] = Array.from(table.querySelectorAll('th, td'));
    
    // 存储所有单元格的起始位置
    const cellPositions: {element: HTMLTableCellElement, start: number, end: number}[] = [];
    ...
    cellPositions.forEach(cellPos => {
      ...
      // 查找与此单元格重叠的问题
      const cellIssues = validIssues.filter(issue => 
        issue.startPosition < end && issue.endPosition > start
      );
      ...
      if (cellIssues.length > 0) {
        ...
        const sortedCellIssues = [...cellIssues].sort(
          (a, b) => Math.max(a.startPosition, start) - Math.max(b.startPosition, start)
        );
  ...
  const renderHighlightedContent = () => {
    ...
    // 问题排序(按照在文本中出现的顺序)
    const sortedIssues = [...issues].sort((a, b) => a.startPosition - b.startPosition);
    ...
    if (containsHtmlTable) {
      ...
      splitContent.forEach((part, splitIndex) => {
        ...
        // 对当前部分寻找相关的问题
        const partIssues = validIssues.filter(issue => 
            ...
            if (issueStart > lastIssueEnd) {
              parts.push(
                <span key={`text-${splitIndex}-${issue.id}-before`}>
                  {part.substring(lastIssueEnd - position, issueStart - position)}
                </span>
              );
            }
      ...
      if (lastEnd < contract.content.length) {
        parts.push(
          <span key="text-last">
            {contract.content.substring(lastEnd)}
          </span>
        );
      }
    }
  ...
  // 开发调试用，输出问题区域
  useEffect(() => {
    console.log('问题列表:', issues.map(issue => ({
      id: issue.id,
    ...
    <div className="contract-panel">
      <div className="contract-header">
        ...
        <div className="contract-meta">
          <span>上传日期: {new Date(contract.dateUploaded).toLocaleDateString()}</span>
        </div>
      </div>
